...
// BASE_URL keeps assets valid in local Vite and GitHub Pages (/NE5-Deskpet-NEO/).
const asset = (path: string) => `${import.meta.env.BASE_URL}assets/jirai/${path}`

export const EXPRESSION_ASSET: Record<JiraiEmotion,string> = {
  neutral: asset('expressions/neutral.png'),
  happy: asset('expressions/happy.png'),
  wink: asset('expressions/wink.png'),
  surprised: asset('expressions/surprised.png'),
  sad: asset('expressions/sad.png'),
  annoyed: asset('expressions/annoyed.png'),
  sleepy: asset('expressions/sleepy.png'),
  excited: asset('expressions/excited.png')
}

export const MOUTHLESS_ASSET = asset('expressions/neutral_mouthless.png')
export const VISEME_ASSET: Record<Viseme,string> = Object.fromEntries(
  (['REST','SMILE','A','E','I','O','U'] as Viseme[]).map(v => [v, asset(`visemes/${v}.png`)])
