...
const endpointBlend=computed(()=>speaking.value?0:Math.max(0,Math.min(1,(effectiveEmotionProgress.value-.72)/.28)))
...
function spriteStyle(rect:SpriteRect){return{width:`${rect.w}px`,height:`${rect.h}px`,backgroundImage:`url(${frame.value.atlasSrc})`,backgroundRepeat:'no-repeat',backgroundPosition:`-${rect.x}px -${rect.y}px`,backgroundSize:`${PARTS_ATLAS_SIZE.width}px ${PARTS_ATLAS_SIZE.height}px`}}
...
onMounted(()=>{last=performance.now();fpsSince=last;emotionStarted=last-transitionMs.value;visemeStarted=last-visemeMs;const q=new URLSearchParams(location.search),qaEmotion=q.get('qaEmotion') as JiraiEmotion|null;if(qaEmotion&&JIRAI_EMOTIONS.includes(qaEmotion)){fromEmotion.value=qaEmotion;toEmotion.value=qaEmotion;emotionProgress.value=1;qaProgress.value=1;qaPaused.value=true;now.value=1}if(q.get('qaReference')==='1')showReference.value=true;raf=requestAnimationFrame(tick)});onBeforeUnmount(()=>cancelAnimationFrame(raf))
</script>

<template>
<main class="app-shell">
  <section class="hero-panel">
    <header class="topbar"><div><p class="eyebrow">NE5 DESKPET NEO / JIRAI CUTOUT RIG V3.3</p><h1>Part-rig Character Simulator</h1></div><div class="metrics"><span><i class="dot"></i>{{fps}} FPS</span><span>Reference-locked endpoints</span><span>CSS sprite crop</span></div></header>
    <div class="stage jirai-stage">
      <div class="character-frame" data-testid="character-frame">
        <div class="rig-html" :aria-label="frame.statusLabel" data-testid="rig-output" :data-emotion="frame.emotion" :data-mouth-layers="frame.mouthSprites.length" :data-gesture-layers="frame.gestures.length">
          <div class="rig-root" data-testid="rig-root" :style="{transform:`translateY(${frame.rootY}px) rotate(${frame.rootRotation}deg)`}">
            <img class="rig-base" :src="frame.baseSrc" alt="" draggable="false"/>
            <svg class="rig-vector-layer" viewBox="0 0 351 345" aria-hidden="true"><g class="brows" :opacity="frame.browOpacity"><path :d="frame.browLeft"/><path :d="frame.browRight"/></g><path v-if="frame.mouthVectorPath&&frame.mouthVectorVisible" :d="frame.mouthVectorPath" class="rig-mouth-line" data-testid="mouth-vector"/></svg>
            <div v-for="(eye,i) in frame.eyes" :key="`eye-${i}`" class="sprite-slot eye-slot" :data-testid="`eye-open-${i}`" :style="{left:`${eye.x}px`,top:`${eye.y}px`,width:`${eye.rect.w}px`,height:`${eye.rect.h}px`,transform:`translate(-50%,-50%) rotate(${eye.rotation}deg)`,clipPath:`inset(${Math.max(0,(1-eye.reveal)*50)}% 0 ${Math.max(0,(1-eye.reveal)*50)}% 0)`,opacity:eye.reveal}"><div class="atlas-sprite" :style="spriteStyle(eye.rect)"></div></div>
            <div v-for="(eye,i) in frame.eyes" :key="`closed-${i}`" class="sprite-slot closed-eye-slot" :data-testid="`eye-closed-${i}`" :style="{left:`${eye.x}px`,top:`${eye.y}px`,width:`${eye.closedRect.w}px`,height:`${eye.closedRect.h}px`,transform:'translate(-50%,-50%)',opacity:eye.closedOpacity,pointerEvents:'none'}"><div class="atlas-sprite" :style="spriteStyle(eye.closedRect)"></div></div>
            <div v-for="(mouth,i) in frame.mouthSprites" :key="`mouth-${i}`" class="sprite-slot mouth-slot" :data-testid="`mouth-sprite-${i}`" :style="{left:`${mouth.x}px`,top:`${mouth.y}px`,width:`${mouth.rect.w}px`,height:`${mouth.rect.h}px`,transform:'translate(-50%,-50%)',clipPath:`inset(${Math.max(0,(1-mouth.reveal)*50)}% 0 ${Math.max(0,(1-mouth.reveal)*50)}% 0)`,opacity:mouth.reveal}"><div class="atlas-sprite" :style="spriteStyle(mouth.rect)"></div></div>
            <div v-for="(gesture,i) in frame.gestures" :key="`gesture-${i}`" class="sprite-slot gesture-slot" :data-testid="`gesture-${i}`" :style="{left:`${gesture.x}px`,top:`${gesture.y+gesture.offsetY}px`,width:`${gesture.rect.w}px`,height:`${gesture.rect.h}px`,transform:'translate(-50%,-50%)'}"><div class="atlas-sprite" :style="spriteStyle(gesture.rect)"></div></div>
            <svg class="endpoint-lock" data-testid="endpoint-lock" viewBox="0 0 351 345" :style="{position:'absolute',inset:'0',width:'351px',height:'345px',zIndex:20,pointerEvents:'none',opacity:endpointBlend}"><image :href="frame.referenceSrc" :x="frame.referenceView.x" :y="frame.referenceView.y" :width="frame.referenceView.width" :height="frame.referenceView.height" preserveAspectRatio="none"/></svg>
          </div>
        </div>
        <svg v-if="showReference" class="reference-overlay" data-testid="reference-overlay" viewBox="0 0 351 345"><image :href="frame.referenceSrc" :x="frame.referenceView.x" :y="frame.referenceView.y" :width="frame.referenceView.width" :height="frame.referenceView.height" preserveAspectRatio="none"/></svg>
      </div>
      <div class="state-card"><span>HYBRID CUTOUT OUTPUT</span><strong>{{frame.statusLabel}}</strong><small>{{fromEmotion}} → {{toEmotion}} · {{Math.round(effectiveEmotionProgress*100)}}% · endpoint {{Math.round(endpointBlend*100)}}%</small></div>
    </div>
  </section>

  <aside class="control-panel">
    <div class="panel-heading"><div><p class="eyebrow">RIG V3.3 CONTROLS</p><h2>Natural expression transition</h2></div><button class="primary" @click="playDemo">Run transition demo</button></div>
    <section class="control-section"><div class="section-title"><span>01</span><h3>Reference expressions</h3></div><div class="chip-grid emotion-grid"><button v-for="e in JIRAI_EMOTIONS" :key="e" :data-emotion-button="e" :class="{active:toEmotion===e&&!speaking}" @click="speaking=false;setEmotion(e)">{{e}}</button></div><label class="slider-row"><span>Transition duration <b>{{transitionMs}} ms</b></span><input v-model.number="transitionMs" type="range" min="220" max="1200" step="20"/></label></section>
    <section class="control-section"><div class="section-title"><span>02</span><h3>Transition QA scrub</h3></div><div class="renderer-dock"><button @click="previewTransition('neutral','happy')">Neutral → Happy</button><button @click="previewTransition('neutral','sad')">Neutral → Sad</button></div><label class="toggle"><input v-model="qaPaused" type="checkbox"/>Pause transition for frame inspection</label><label class="slider-row"><span>Transition frame <b>{{Math.round(qaProgress*100)}}%</b></span><input v-model.number="qaProgress" :disabled="!qaPaused" type="range" min="0" max="1" step=".01"/></label><p class="qa-copy">The cutout rig drives the transition; the final 28% blends into the supplied expression endpoint so body pose, blush and face details remain reference-accurate.</p></section>
