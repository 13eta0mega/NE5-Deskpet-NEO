...
async function ready(page,emotion,reference=false){
  ...
  await page.addStyleTag({content:'*,*::before,*::after{animation:none!important;transition:none!important}.rig-root{transform:none!important}'})
...
async function assertVisibleParts(page,emotion){
  ...
  const eyeBoxes=await root.locator('.eye-slot').evaluateAll(nodes=>nodes.map(n=>{const r=n.getBoundingClientRect(),s=getComputedStyle(n);return{w:r.width,h:r.height,opacity:Number(s.opacity)}}))
  for(const eye of eyeBoxes){expect(eye.w).toBeGreaterThan(20);expect(eye.h).toBeGreaterThan(8);expect(eye.opacity).toBeGreaterThan(.95)}
  const mouthBoxes=await root.locator('.mouth-slot').evaluateAll(nodes=>nodes.map(n=>{const r=n.getBoundingClientRect(),s=getComputedStyle(n);return{w:r.width,h:r.height,opacity:Number(s.opacity)}}))
  expect(mouthBoxes.length).toBe(expected[emotion].mouth)
  for(const mouth of mouthBoxes){expect(mouth.w).toBeGreaterThan(8);expect(mouth.h).toBeGreaterThan(6);expect(mouth.opacity).toBeGreaterThan(.99)}
  const gestureBoxes=await root.locator('.gesture-slot').evaluateAll(nodes=>nodes.map(n=>{const r=n.getBoundingClientRect();return{w:r.width,h:r.height}}))
  expect(gestureBoxes.length).toBe(expected[emotion].gesture)
}
...
test.describe('Jirai strict rendered QA',()=>{
  for(const emotion of emotions)test(`${emotion}: isolated rendered endpoint matches isolated normalized reference`,async({page},testInfo)=>{
    ...
    if(ratio>limit){
      const ap=testInfo.outputPath(`${emotion}-actual.png`),rp=testInfo.outputPath(`${emotion}-reference.png`),dp=testInfo.outputPath(`${emotion}-diff.png`)
      fs.writeFileSync(ap,actual);fs.writeFileSync(rp,reference);fs.writeFileSync(dp,diff)
      await testInfo.attach('actual',{path:ap,contentType:'image/png'});await testInfo.attach('reference',{path:rp,contentType:'image/png'});await testInfo.attach('diff',{path:dp,contentType:'image/png'})
    }
